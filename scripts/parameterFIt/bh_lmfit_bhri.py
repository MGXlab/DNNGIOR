...
def pseudoHuberLoss(y_true, y_pred, delta = 1.0):
    ...
    choice = np.random.choice(np.arange(len(y_true)), size = int(0.7*len(y_true)))
...
def distance(lmfit_params, database):
    ...
    distance = np.array([live_distance,
                         dead_distance,
                         pH_distance,
                         trehalose_distance,
                         pyruvate_distance,
...
inputParams = getPramsFromFile('bh', os.path.join(Path(os.getcwd()).parents[1], 'files', 'params', 'bh1.tsv'))


outputFile = os.path.join(Path(os.getcwd()).parents[1], 'files', 'params', 'bh1.tsv')
...
d = distance(inputParams, database)
