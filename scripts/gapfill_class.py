...
class Gapfill:
    def __init__(self, draftModelPath, trainedNNPath = None, medium = None, objectiveName = 'bio1', dbType = 'ModelSEED'):
        ...
        #different reaction database
        if dbType=='ModelSEED':
            self.path_to_biochem  = os.path.join(self.files_path, 'biochemistry', 'reactions.tsv')
        else:
            self.path_to_biochem  = os.path.join(self.files_path, 'biochemistry', 'bigg_reactions.tsv')
        ...
        self.exchange_reacs = Reaction( model = os.path.join(self.models_path, 'exchangeReactions.sbml'),
                          fixed_bounds = self.medium, dbType=self.dbType)
        ...
        # predict weights
        p = NN(path = self.trainedNNPath, modeltype=self.dbType).predict( self.draft_reaction_ids)
    ...
    def gapfill(self,
        ...
        #print('size all reacs:',len(all_reacs))
