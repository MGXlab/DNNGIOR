...
model_location = os.path.join(path, 'dnngior', 'files',  'models', 'all_bins-draft.xml')

# EXPLAIN THE REACTION CLASS
draft_model = Reaction(model=model_location) 

# Set of reaction ids in draft model.
draft_reaction_ids = set(draft_model.reactions) 

# Load the database with reactions used for gapfilling
biochem = os.path.join(path, 'dnngior', 'files',  'biochemistry', 'reactions.tsv')
...
gapfilled_model_location = os.path.join(path, 'dnngior',  'models', 'all_bins-draft_gpfilled.xml')
...
media_file = os.path.join(path, 'dnngior', 'files', 'biochemistry', 'Nitrogen-Nitrite_media.tsv')
